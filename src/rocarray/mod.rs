...
use crate::hip::{DeviceMemory, Stream, Event, Timer, calculate_grid_1d, Dim3};
    ...
    T: Copy + Default + 'static,
{
    ...
    pub fn get(&self, indices: &[usize]) -> Result<T> {
        let flat_index = self.shape.ravel_index(indices)
            .ok_or_else(|| crate::error::Error::InvalidOperation(
    ...
    pub fn to_vec_async(&self, stream: &Stream) -> crate::hip::Result<PendingCopy<T>> {
